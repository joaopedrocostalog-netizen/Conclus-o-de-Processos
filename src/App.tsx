...
async function readPdf(file:File, kind:DocKind):Promise<ReadResult> {
...
function cnpjFromNf(p?:PdfPage) {
  if (!p) return result(null,null,'NF Fiscal não enviada','low');
  const m=p.text.match(/TRANSPORTADOR\s*\/\s*VOLUMES\s*TRANSPORTADOS[\s\S]{0,900}?CNPJ\s*(?:\/\s*CPF)?[\s\S]{0,120}?([0-9]{2}\.?[0-9]{3}\.?[0-9]{3}\/?[0-9]{4}-?[0-9]{2})/i);
  if (m?.[1]) return result(formatCnpj(m[1]),p.page,`NF FISCAL · página 1 · Transportador / CNPJ-CPF: ${formatCnpj(m[1])}`);
  const all=[...p.text.matchAll(/\b(\d{2}\.\d{3}\.\d{3}\/\d{4}-\d{2})\b/g)];
...
function weightFromDoc(p?:PdfPage) {
  if (!p) return result(null,null,'DOC COMPLETO · página 5 não disponível','low');
  const label=/Peso\s+L[ií]quido\s*\(kg\)\s*:/i;
  const idx=p.text.search(label);
  if (idx<0) return result(null,null,'DOC COMPLETO · página 5: “Peso Líquido (kg)” não localizado','low');
  const window=p.text.slice(idx,idx+220);
  const candidates=[...window.matchAll(/\b(\d{1,3}(?:\.\d{3})*,\d{3,5}|\d{4,},\d{3,5})\b/g)].map(m=>m[1]);
  const value=candidates.find(v=>numberBR(v)>0) || null;
  return value ? result(`${value} kg`,5,`DOC COMPLETO · página 5 · Peso Líquido (kg): ${value}`) : result(null,null,'DOC COMPLETO · página 5: valor do Peso Líquido não localizado','low');
}
...
function totalFromNf(p?:PdfPage) {
  if (!p) return result(null,null,'NF Fiscal não enviada','low');
  const idx=p.text.search(/VALOR\s+TOTAL\s+DA\s+NOTA/i);
  if (idx<0) return result(null,null,'NF FISCAL · página 1: “VALOR TOTAL DA NOTA” não localizado','low');
  const window=p.text.slice(idx,idx+500);
  const candidates=[...window.matchAll(/\b(\d{1,3}(?:\.\d{3})*,\d{2}|\d{4,},\d{2})\b/g)].map(m=>m[1]).filter(v=>numberBR(v)>0);
  if (!candidates.length) return result(null,null,'NF FISCAL · página 1: valor após “VALOR TOTAL DA NOTA” não localizado','low');
  const value=[...candidates].sort((a,b)=>numberBR(b)-numberBR(a))[0];
  return result(`R$ ${value}`,1,`NF FISCAL · página 1 · VALOR TOTAL DA NOTA: R$ ${value}`);
}

function analyzeDocuments(results:ReadResult[]):Analysis {
  const all=results.flatMap(r=>r.pages);
  const doc=results.find(r=>r.kind==='DOC COMPLETO');
  const nf=results.find(r=>r.kind==='NF FISCAL');
  const docPages=doc?.pages||[], nfPages=nf?.pages||[];
  const doc1=docPages.find(p=>p.page===1), doc5=docPages.find(p=>p.page===5), nf1=nfPages.find(p=>p.page===1);
  ...
  return { process_type:processType, fields, pages:results.reduce((a,r)=>a+r.total,0), filename:results.map(r=>r.name).join(' + '), ocr_pages:all.filter(p=>p.ocr).map(p=>`${p.document} p.${p.page}`), documents:results.map(r=>r.kind) };
}
