...
function exporterFromDuimp(p:Page){
  ...
  for(const source of sources){
    const explicit=source.match(/\bOPE[_\s-]*\d+\s*-\s*([A-Z0-9][A-Z0-9 .,&'()\/-]{2,100}?)(?=\s+(?:VERS[AÃ]O|VERSAO|ENDERE[CÇ]O|DADOS\s+DA\s+MERCADORIA|APLICA[CÇ][AÃ]O|FABRICANTE|PA[IÍ]S\b)|$)/i);
    if(explicit?.[1]){const value=cleanParty(explicit[1]);if(!invalidParty(value))return value}
  }
  for(let i=0;i<p.rows.length;i++){
    const row=p.rows[i];
    const same=row.match(/C[oó]digo do Exportador Estrangeiro\s*:?\s*(?:OPE[_\s-]*\d+\s*-\s*)?(.+)$/i);
    if(same?.[1]){const value=cleanParty(same[1]);if(!invalidParty(value)&&!/^(VERS[AÃ]O|VERSAO)/i.test(value))return value}
    if(!/C[oó]digo do Exportador Estrangeiro/i.test(row))continue;
    for(const next of p.rows.slice(i+1,i+7)){
      const m=next.match(/(?:^|\s)OPE[_\s-]*\d+\s*-\s*(.+)$/i);
...
function moneyCandidates(text:string){
  ...
  return matches.map(normalizeMoney).filter(Boolean);
}
function valorNota(pages:Page[]):Pick{
  for(const p of pagesOf(pages,'NF-e')){
    for(let i=0;i<p.rows.length;i++){
      const row=p.rows[i];
      const label=/VALOR TOTAL DA NOTA/i;
      if(!label.test(row))continue;
      const afterLabel=row.replace(/^.*?VALOR TOTAL DA NOTA\s*[:\-]?\s*/i,'');
      const onSameRow=moneyCandidates(afterLabel);
      if(onSameRow.length)return picked(onSameRow[onSameRow.length-1],p,'VALOR TOTAL DA NOTA na NF-e');
      for(const next of p.rows.slice(i+1,i+3)){
        if(/TRANSPORTADOR\/VOLUMES TRANSPORTADOS|NOME\/RAZ[AÃ]O SOCIAL|VALOR TOTAL DOS PRODUTOS|VALOR DO FRETE|VALOR DO SEGURO|DESCONTO|OUTRAS DESPESAS|VALOR TOTAL DO IPI/i.test(next))break;
        const values=moneyCandidates(next);
        if(values.length)return picked(values[values.length-1],p,'VALOR TOTAL DA NOTA na NF-e');
      }
    }
    const textMatch=p.text.match(/VALOR TOTAL DA NOTA[^\n]*\n\s*([^\n]+)/i);
    if(textMatch?.[1]){
      const values=moneyCandidates(textMatch[1]);
      if(values.length)return picked(values[values.length-1],p,'VALOR TOTAL DA NOTA na NF-e');
    }
  }
